...
class Config:
    ...
    IS_PRODUCTION = ENVIRONMENT == 'production' or bool(os.getenv('RAILWAY_ENVIRONMENT')) or bool(os.getenv('PORT'))
    
    # OpenAI API Configuration - NEVER hardcode keys!
    # Primary fallback order for API keys
    OPENAI_API_KEY = (
        os.getenv('OPENAI_API_KEY_LARGE') or 
        os.getenv('OPENAI_API_KEY_NANO') or 
        os.getenv('OPENAI_API_KEY')
    )
    
    # Agent-specific API Keys for optimal performance
    AGENT1_API_KEY = os.getenv('OPENAI_API_KEY_NANO') or OPENAI_API_KEY    # Nano key for gpt-4o-mini
    AGENT3_API_KEY = os.getenv('OPENAI_API_KEY_LARGE') or OPENAI_API_KEY   # Large context key for gpt-4o
    ...
    def validate_config(cls):
        ...
        if not cls.OPENAI_API_KEY:
            ...
            print("- OPENAI_API_KEY_LARGE")
            print("- OPENAI_API_KEY_NANO") 
            print("- OPENAI_API_KEY")
            raise ValueError("OpenAI API key is required")
        
        # Validate API keys
        if not cls.AGENT1_API_KEY:
            print("⚠️ Warning: No Agent 1 API key, using fallback")
        if not cls.AGENT3_API_KEY:
            print("⚠️ Warning: No Agent 3 API key, using fallback")
        
        print(f"✅ Environment: {cls.ENVIRONMENT}")
        print(f"✅ Production mode: {cls.IS_PRODUCTION}")
        print(f"✅ API keys configured: Agent1={bool(cls.AGENT1_API_KEY)}, Agent3={bool(cls.AGENT3_API_KEY)}")
